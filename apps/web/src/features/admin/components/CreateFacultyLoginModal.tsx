...
import { useCreateFacultyLogin } from '../hooks/useAdminFaculty';
...
export function CreateFacultyLoginModal({ faculty, onClose }: Props) {
  const [loginEmail, setLoginEmail] = useState(faculty.email ?? '');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  const { mutateAsync, isPending } = useCreateFacultyLogin();

  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    setError('');
    try {
      const result = await mutateAsync({ id: faculty._id, payload: { loginEmail, password } });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <form onSubmit={handleSubmit} className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        ...
        <p className="text-sm text-gray-500 mb-5">For {faculty.fullName}</p>
